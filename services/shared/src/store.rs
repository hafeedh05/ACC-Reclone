...
impl MemoryStore {
        ...
        let storyboard = StoryboardDraft {
            ...
            scenes: vec![
                SceneSpec {
                    ...
                    index: 1,
                    duration_seconds: 5,
                    visual_direction: "cinematic opener".to_string(),
                    camera_direction: "push in".to_string(),
                    prompt: "Start with the strongest proof point".to_string(),
                },
                SceneSpec {
                    id: new_id("scene"),
                    index: 2,
                    duration_seconds: 6,
                    visual_direction: "product close-up".to_string(),
                    camera_direction: "steady glide".to_string(),
                    prompt: "Show the offer and payoff".to_string(),
                },
            ],
        };
    ...
    pub fn regenerate_script(&self, run_id: &str) -> ServiceResult<GenerationRun> {
        ...
        script.version += 1;
        script.headline = format!("{} v{}", script.headline, script.version);
    ...
    pub fn regenerate_storyboard(&self, run_id: &str) -> ServiceResult<GenerationRun> {
        ...
        let run = inner
            ...
            .get_mut(run_id)
            .ok_or_else(|| ServiceError::NotFound(format!("run {run_id}")))?;
        let mut storyboard = run.storyboard.clone().ok_or_else(|| {
            ServiceError::InvalidRequest("storyboard not initialized".to_string())
        })?;
        storyboard.version += 1;
        if let Some(first_scene) = storyboard.scenes.first_mut() {
            first_scene.prompt = format!("{} - refined", first_scene.prompt);
        }
